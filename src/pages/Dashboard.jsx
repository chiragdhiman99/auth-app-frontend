...
export default function Dashboard() {
    ...
    const fetchUser = async () => {
      try {
        ...
        const res = await axios.get(
          "https://auth-app-backend-09tp.onrender.com/api/auth/me",
          {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white flex relative overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/5 backdrop-blur-xl border-r border-white/10 z-30 flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <main className="flex-1 p-4 lg:p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat) => (
              <div
                key={stat.label}
                className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-5 hover:border-violet-500/30 transition-all duration-200 group"
              >
            ...
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  {
                    label: "Login Method",
                    value: userData?.googleId ? "Google" : "Email",
                  },
                  ...
                  <div key={i} className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs font-medium ${item.green ? "text-emerald-400" : item.red ? "text-red-400" : "text-white/70"}`}
                    >
                      {item.value}
                    </span>
                  </div>
